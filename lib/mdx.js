...
export async function getMdxPreviews(basePath, type, maxLength = 500, contentType = 'musings') {
  ...
  const previews = await Promise.all(
    files.map(async (file) => {
      ...
      // console.log(`File: ${file}, Frontmatter:`, frontmatter)

      // Skip if incorrect content_type
      if (!frontmatter.content_type || !frontmatter.content_type.includes(contentType)) {
        console.log(`Skipping ${file}: Incorrect content_type (${frontmatter.content_type})`)
        return null
      }
      ...
      const cleanText = removeMarkdown(content).replace(/<\/?[^>]+(>|$)/g, '')

      const truncatedContent = cleanText.length > maxLength ? cleanText.slice(0, maxLength) + '...' : cleanText

      return {
        previewText: truncatedContent, // ✅ Now returning clean text instead of MDX
        frontMatter: {
