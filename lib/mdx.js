...
import remarkImageSyntax from './remark-image-syntax'
// Rehype packages
...
// getFileBySlug - Returns: An object containing the processed MDX source, a table of contents, and frontmatter (metadata) of the file.
export async function getFileBySlug(type, slug) {
  const mdxPath = path.join(root, pathData.root, type, `${slug}.mdx`)
  const mdPath = path.join(root, pathData.root, type, `${slug}.md`)
...
//getDataFile - Almost identical to getFileBySlug - returns an object with processed .mdx data. But checks the /data directory instead of /content (used for authors or page content instead of posts)
export async function getDataFile(type, slug) {
...
//getAllFilesFrontMatterMultiple - Same as above but takes multiple types
export async function getAllFilesFrontMatterMultiple(types) {
  let allFrontMatter = []
    ...
    files.forEach((file) => {
      ...
      if (frontmatter.draft !== true) {
        allFrontMatter.push({
          ...frontmatter,
          slug: formatSlug(fileName),
          directory: type, // Use the 'type' as the directory name
          date: frontmatter.date ? new Date(frontmatter.date).toISOString() : null,
        })
      }
    })
