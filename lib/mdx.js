...
export async function getAllFilesByCategory(categories) {
  ...
  const files = getAllFilesRecursively(blogContentRoot)

  console.log(`Total files found: ${files.length}`) // Debug log 1

  files.forEach((file) => {
    console.log(`Processing file: ${file}`) // Debug log 2
    ...
    // Check if the post's category intersects with any of the given categories
    if (
      frontmatter.draft !== true &&
      frontmatter.category &&
      frontmatter.category.some((cat) => categories.includes(cat))
    ) {
      console.log(`File matches categories '${categories.join(', ')}': ${file}`) // Adjusted for multiple categories

      const filePath = file.substring(blogContentRoot.length + 1)
      const formattedFilePath = filePath.replace(/\\/g, '/')

      allFrontMatter.push({
        ...frontmatter,
        slug: formattedFilePath.replace(path.extname(formattedFilePath), ''),
        date: frontmatter.date ? new Date(frontmatter.date).toISOString() : null,
      })
    }
  })

  console.log(`Total posts found: ${allFrontMatter.length}`) // Debug log 4
