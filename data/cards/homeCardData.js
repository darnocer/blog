import LaptopIcon from '@/components/icons/components/LaptopIcon'
import SparklesIcon from '@/components/icons/components/SparklesIcon'
import MicrodoseIcon from '@/components/icons/components/MicrodoseIcon'
import PencilIcon from '@/components/icons/components/PencilIcon'
import PsychedelicsIcon from '@/components/icons/components/PsychedelicsIcon'
import MeditationIcon from '@/components/icons/components/MeditationIcon'
import SoulScienceIcon from '@/components/icons/components/SoulScienceIcon'

const homeCardData = [
  { title: 'Soul Science', link: 'https://www.soulsciencehq.com', icon: <SoulScienceIcon /> },
  { title: 'Microdose.wiki', link: 'https://www.microdose.wiki', icon: <MicrodoseIcon /> },
  {
    title: 'darian.digital',
    link: 'https://drive.google.com/file/d/1K15gxW-LqSCN0RbsmkotsjSpPwyvikh_/view?usp=sharing',
    icon: <LaptopIcon />,
  },
]

export default homeCardData
